...
By default we assume that the user will supply text files corresponding to individual Kaplan-Meier curves. That is, each file will have columns corresponding to `"time"`, `"n.risk"`, `"n.event"`, `"n.censor"`. Note that the columns __do not have to follow this naming convention, but have to follow this ordering.__)
...
Plotting Kaplan-Meier curves relies on external packages `survival` and `survminer` (function `ggsurvplot`). These will be installed automatically when user installs `survivalnma`.
...
```{r survival, , fig.width= 8, fig.height= 5}
# read data
...
```{r}
second.order.nma <- survnma(dataframe, "fp2", P = c(2.3, 0.23), type = "random", min_time_change = 0.05)
# The output "remembers"" all this extra info
second.order.nma$model
second.order.nma$P
```
